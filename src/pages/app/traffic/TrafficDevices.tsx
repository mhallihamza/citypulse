  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <MetricCell label="Overdue" value={state?.overdueVehicles ?? "—"} />
        <MetricCell label="Density" value={state?.density != null ? Number(state.density).toFixed(1) : "—"} />
        <MetricCell label="T-max" value={state?.tmax != null ? `${Number(state.tmax).toFixed(0)}s` : "—"} />
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.55fr_1fr]">
        <div className="space-y-5">
          <Card>
            <CardHeader title="Telemetry history" subtitle="Real rows from traffic_telemetry" />
            <CardBody>
              {samples.length === 0 ? (
                <EmptyState title="No telemetry data available." message={`traffic_telemetry has no samples for ${device.deviceKey} yet. Values are never generated by the UI.`} />
              ) : (
                <div className="grid gap-4 md:grid-cols-2">
                  <TelemetryChart title="Vehicle count" color="#8B5CF6" field="vehicleCount" samples={samples} fmt={(v) => v.toFixed(0)} />
                  <TelemetryChart title="Density" color="#246BFF" field="density" samples={samples} fmt={(v) => v.toFixed(1)} />
                  <TelemetryChart title="Overdue vehicles" color="#EF4444" field="overdueVehicles" samples={samples} fmt={(v) => v.toFixed(0)} />
                  <TelemetryChart title="T-max (s)" color="#F59E0B" field="tmax" samples={samples} fmt={(v) => `${v.toFixed(0)}s`} />
