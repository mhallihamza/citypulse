...
/** Traffic device registry — backed by the shared `devices` table. */
export function TrafficDevicesPage() {
  const { devices, trafficStates, now } = useApp();
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState(false);
  const fleet = useMemo(() => devices.filter((d) => d.service === "traffic"), [devices]);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Registered road-segment controllers — stored in Supabase, fed by the IoT pipeline."
        actions={
                ...
                <tr>
                  <td colSpan={7}>
                    <EmptyState
                      ...
                      message="Register a traffic segment controller to start receiving live data from the IoT pipeline."
                ...
                return (
                  <tr key={d.id} className="transition-colors hover:bg-ink-50/50">
                    <td className="px-5 py-3">
                      <Link to={`/app/traffic/devices/${d.id}`} className="font-mono text-[13px] font-semibold text-pulse-600 hover:underline">{d.deviceKey}</Link>
                    ...
                    <td className="px-4 py-3 tabular text-ink-700">{s?.vehicleCount ?? "—"}</td>
                    <td className="px-4 py-3">{s ? <ServiceStatePill state={s.state} /> : <span className="text-xs text-ink-400">—</span>}</td>
                    <td className="px-4 py-3 text-xs tabular text-ink-500">{s?.lastSeen ? timeAgo(s.lastSeen, now) : "never"}</td>
                    <td className="px-4 py-3 text-right">
                      <Link to={`/app/traffic/devices/${d.id}`}><Button variant="ghost" size="xs">Open</Button></Link>
                    </td>
                  </tr>
                );
              })}
...
/** Traffic segment detail — identity, live state, telemetry history, events, tickets. */
export function TrafficDeviceDetail() {
  const { deviceId = "" } = useParams();
  const { devices, trafficStates, telemetry, events, tickets, now } = useApp();
  const device = devices.find((d) => d.id === deviceId && d.service === "traffic");
  const state = trafficStates[deviceId];
  ...
  if (!device) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="mt-1 text-sm text-ink-500">No traffic device with this id exists in your organization.</p>
        <Link to="/app/traffic/devices" className="mt-4 inline-block">
  ...
  return (
    <div>
      <Link to="/app/traffic/devices" className="mb-3 inline-flex items-center gap-1.5 text-sm font-medium text-ink-500 hover:text-pulse-600">
        ← Back to Traffic devices
      </Link>
      ...
      <PageHeader
        ...
        subtitle={`TRAFFIC · ${device.type}${device.locationLabel ? ` · ${device.locationLabel}` : ""}`}
        actions={<OnlineBadge online={Boolean(state?.online)} />}
      />

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <MetricCell label="Vehicle count" value={state?.vehicleCount ?? "—"} />
        <MetricCell label="Pending" value={state?.pendingVehicles ?? "—"} />
        <MetricCell label="Density" value={state?.density != null ? Number(state.density).toFixed(1) : "—"} />
        <MetricCell label="Congestion" value={state?.congestion != null ? `${Number(state.congestion).toFixed(0)}%` : "—"} />
        <MetricCell label="Travel time" value={state?.travelTime != null ? `${Number(state.travelTime).toFixed(0)}s` : "—"} />
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.55fr_1fr]">
        <div className="space-y-5">
          <Card>
            <CardHeader title="Telemetry history" subtitle="Real rows from device_telemetry" />
            <CardBody>
              {samples.length === 0 ? (
                <EmptyState title="No telemetry data available." message={`device_telemetry has no samples for ${device.deviceKey} yet. Values are never generated by the UI.`} />
              ) : (
                <div className="grid gap-4 md:grid-cols-2">
                  <TelemetryChart title="Vehicle count" color="#8B5CF6" field="vehicles" samples={samples} fmt={(v) => v.toFixed(0)} />
                  <TelemetryChart title="Density" color="#246BFF" field="density" samples={samples} fmt={(v) => v.toFixed(1)} />
                  <TelemetryChart title="Congestion %" color="#EF4444" field="congestion" samples={samples} fmt={(v) => `${v.toFixed(0)}%`} />
                  <TelemetryChart title="Travel time (s)" color="#F59E0B" field="travelTime" samples={samples} fmt={(v) => `${v.toFixed(0)}s`} />
                  <div className="col-span-full text-[11px] text-ink-400 tabular">Latest sample {timeAgo(samples[samples.length - 1].ts, now)}</div>
                </div>
              )}
            </CardBody>
          </Card>
              ...
              {deviceEvents.map((e) => (
                <div key={e.id} className="flex items-start gap-3 px-5 py-3.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-ink-400">{e.eventType} · {timeAgo(e.createdAt, now)} · source: {e.source}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <CardBody className="space-y-2.5 text-sm">
              ...
              <Row label="Service" value="TRAFFIC" />
