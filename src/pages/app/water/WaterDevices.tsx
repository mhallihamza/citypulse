          ...
          <table className="w-full min-w-[780px] text-sm">
            <thead>
              <tr className="border-b border-ink-100 bg-ink-50/60 text-left text-[11px] uppercase tracking-widest text-ink-400">
                ...
                <th className="px-4 py-3 font-semibold">Leakage</th>
              ...
              {filtered.map((d) => {
                ...
                return (
                  <tr key={d.id} className="transition-colors hover:bg-ink-50/50">
                    ...
                    <td className="px-4 py-3 tabular text-ink-700">{s?.flow != null ? `${Number(s.flow).toFixed(1)} L/s` : "—"}</td>
                    <td className="px-4 py-3">{s ? (s.leakage ? <span className="rounded-full bg-red-50 px-2 py-0.5 text-[11px] font-bold text-red-700">LEAK</span> : <span className="text-xs font-medium text-live-600">None</span>) : "—"}</td>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <MetricCell label="Flow" value={state?.flow != null ? `${Number(state.flow).toFixed(1)} L/s` : "—"} />
        <MetricCell label="Pressure" value={state?.pressure != null ? `${Number(state.pressure).toFixed(2)} bar` : "—"} />
        <MetricCell label="Leakage" value={state?.leakage ? "LEAK" : "None"} tone={state?.leakage ? "text-red-600" : undefined} />
            ...
            <CardBody>
              {samples.length === 0 ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <TelemetryChart title="Flow (L/s)" color="#246BFF" field="flow" samples={samples} fmt={(v) => `${v.toFixed(1)} L/s`} />
                  <TelemetryChart title="Pressure (bar)" color="#8B5CF6" field="pressure" samples={samples} fmt={(v) => `${v.toFixed(2)} bar`} />
